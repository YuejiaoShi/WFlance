...
export default function Header() {
  ...
  const menuItems = [
    ...
    { name: "Payments", path: "/dev-dashboard/payments" },
  ];
    ...
    <header className="flex justify-between items-center bg-primary-blue md:px-4 px-2 py-1 text-white w-full">
      ...
      <div className="flex flex-row justify-between items-center w-full mt-2">
        ...
        <div className="relative flex items-end space-x-4">
          ...
          >
            {devName
              ? devName.split(" ")[0].charAt(0).toUpperCase() +
                devName.split(" ")[0].slice(1).toLowerCase()
              : "?"}
          </div>
          ...
          >
            {({ TransitionProps }) => (
              <ClickAwayListener onClickAway={() => setUserBoxVisible(false)}>
                <Paper
                  {...TransitionProps}
                  className="top-10 left-0 bg-yellow-50 shadow-xl rounded-md p-4"
                >
