...
import {
  ...
  getStatusIcon,
  statusColors,
} from "@/app/_components/DevDashboard/helpers";
import ProgressBar from "@/app/_components/DevDashboard/ProgressBar";
import {
  DeleteButton,
  EditButton,
  PrintInvoicingButton,
} from "@/app/_components/DevDashboard/TableActions";
...
export default function ProjectPage({ params }) {
  ...
  const statusColor =
    statusColors[project.status.toLowerCase()] || statusColors.default;
      ...
      <div className="flow-root md:mx-6 mx-0 bg-white rounded-xl shadow-lg p-6 max-w-full overflow-auto">
        <div className="md:flex-row flex-col md:justify-between flex md:items-center mb-2">
          <div className="flex-col flex mb-4">
            <div className="md:text-3xl text-lg font-semibold text-primary-blue-dark ">
              {project.title}
            </div>
            <div
        ...
        <div className="text-sm md:text-base grid grid-cols-1 md:grid-cols-2 md:gap-6 gap-0 bg-white text-gray-800">
          <div>
            <p className="flex items-center mb-4">
              ...
              <strong className="mr-2 text-base">Client:</strong>{" "}
              {project.client?.name}
            </p>
            <p className="flex items-center mb-4">
              <EuroOutlinedIcon className="mr-3 text-primary-blue-dark text-xl md:text-2xl" />
              <strong className="mr-2 text-base">Budget:</strong> $
              {project.budget}
            </p>
          </div>
          ...
          <div>
            <p className="flex items-center mb-4">
              ...
              <strong className="mr-2 text-base">Start Date:</strong>{" "}
              {project.startDate}
            </p>
            <p className="flex items-center mb-4">
              <CalendarTodayOutlinedIcon className="mr-3 text-primary-blue-dark text-xl md:text-2xl" />
              <strong className="mr-2 text-base">Deadline:</strong>{" "}
              {project.deadline}
            </p>
          </div>
        </div>
        ...
        <div className=" md:p-6 flex justify-center items-center">
          <div className="flex flex-col items-center">
            ...
            <p className=" text-base text-start mt-4 w-full text-gray-600">
